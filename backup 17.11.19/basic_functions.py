...
sta_if.connect('PLUSNET-K727','7a536b38a6')

BLYNK_AUTH = 'DNfNqDRubeXnl5uk215onGsBA8t7JbSN'
b = BlynkLib.Blynk(BLYNK_AUTH)
...
vbutton = 0
def checkButton():
    if (vbutton == 0 and ticks() % 50 == 0) or vbutton == 1:
        @b.on("V1")
        def blynk_handle_vpins(value):
            print("V1 value: {}".format(value))
            global vbutton
            vbutton = int(value[0])
    ...
    if button.value() == 0 or vbutton == 1:
        return True
